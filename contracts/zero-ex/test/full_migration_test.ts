...
blockchainTests.resets('Full migration', env => {
    ...
    before(async () => {
        ...
        await migrator.deploy(owner, zeroEx.address, toFeatureAdddresses(features), { transformerDeployer }).awaitTransactionSuccessAsync();
    });
    ...
    it('Non-deployer cannot call deploy()', async () => {
        ...
        const tx = migrator
            .deploy(owner, zeroEx.address, toFeatureAdddresses(features), { transformerDeployer })
