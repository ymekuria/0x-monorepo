import { ExchangeProxyMetaTransactionData, Order, ZeroExTransaction } from '@0x/types';
 ...
 */
export function getExchangeProxyMetaTransactionHash(mtx: ExchangeProxyMetaTransactionData): string {
    return hexUtils.toHex(
        signTypedDataUtils.generateTypedDataHash(
            eip712Utils.createExchangeProxyMetaTransactionTypedData(mtx),
        ),
    );
}
